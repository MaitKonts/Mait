...
function meiko_fighting_system_shortcode() {
    ...
    $output .= '<div>Your current score in overall ' . $fighting_shortcode_output . '</div>';

    $output .= '<form method="post" action="">
        <label for="fighting_style">Choose Fighting Style:</label>
        <select name="fighting_style">';
    ...
    foreach ($fighting_styles as $style) {
        $output .= '<option value="' . $style->id . '">' . $style->name . '</option>';
    }
