...
function meiko_manage_plants_callback() {
    ...
    if (isset($_POST['submit'])) {
        ...
        // Check if updating an existing plant
    ...
    echo '<h2>Manage Plants</h2>';

    // Display existing plants in a table
    echo '<table class="wp-list-table widefat fixed striped">';
    echo '<thead><tr><th>Name</th><th>Grow Time</th><th>Watering Time</th><th>Price</th><th>Seed Price</th><th>Actions</th></tr></thead>';
    echo '<tbody>';
    foreach ($plants as $plant) {
        echo '<tr>';
        echo '<td>' . esc_html($plant->name) . '</td>';
        echo '<td>' . esc_html($plant->grow_time) . '</td>';
        echo '<td>' . esc_html($plant->watering_time) . '</td>';
        echo '<td>' . esc_html($plant->price) . '</td>';
        echo '<td>' . esc_html($plant->seed_price) . '</td>';
        echo '<td><a href="?page=meiko-plants&edit=' . esc_attr($plant->id) . '">Edit</a> | <a href="?page=meiko-plants&delete=' . esc_attr($plant->id) . '">Delete</a></td>';
        echo '</tr>';
    }
    echo '</tbody>';
    echo '</table>';
    ...
    // Add plant form
    echo '<h3>Add Plant</h3>';
    echo '<form method="post">';
    echo 'Name: <input type="text" name="name" required><br>';
    echo 'Grow Time: <input type="number" name="grow_time" required><br>';
    echo 'Watering Time: <input type="number" name="watering_time" required><br>';
    echo 'Price: <input type="number" step="0.01" name="price" required><br>';
    echo 'Seed Price: <input type="number" step="0.01" name="seed_price" required><br>';
    echo '<input type="submit" name="submit" value="Add Plant">';
    echo '</form>';
}
