...
function meiko_display_current_faction_shortcode($atts) {
    ...
    if ($faction_data['faction_leader'] == $current_user_id) {
        ...
        if ($faction_data['faction_leader'] == $current_user_id) {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['refuse_request'])) {
                ...
                $wpdb->delete($requests_table, ['id' => $user_id_to_refuse]);
            }
        }
    }
    ...
    $output = '<div class="meiko-current-faction-profile">';
    $output .= "<h2>Your Current Faction: {$faction_data['name']}</h2>";
    $output .= $faction_avatar_tag; // Display faction's avatar
    $output .= "<p><strong>Faction Name:</strong> {$faction_data['name']}</p>";
    $output .= "<p><strong>Creation Date:</strong> {$faction_data['creation_date']}</p>";
    $output .= "<p><strong>Faction Leader:</strong> {$leader_name}</p>";
    $output .= "<p><strong>Faction Score:</strong> {$faction_data['score']}</p>";
    $output .= "<p><strong>Defense:</strong> {$faction_data['defense']}</p>";
    $output .= "<p><strong>Attack:</strong> {$faction_data['attack']}</p>";
    $output .= "<p><strong>Defense Equipment Amount:</strong> {$faction_data['defense_equipment']}</p>";
    $output .= "<p><strong>Attack Equipment Amount:</strong> {$faction_data['attack_equipment']}</p>";
    $output .= "<p><strong>Maximum Equipment Amount:</strong> {$faction_data['max_equipment']}</p>";
    $output .= "<p><strong>Faction Money:</strong> {$faction_data['money']}</p>";
    ...
    if ($faction_data['faction_leader'] == $current_user_id) {
        ...
        $output .= '<input type="file" name="faction_avatar_file" id="faction_avatar_file" accept=".png" onchange="checkFileSize(this)">';
        ...
        $output .= '<tr><th>Player</th><th>Action</th></tr>';
