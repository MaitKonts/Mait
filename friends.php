...
function meiko_friends_leaderboard_shortcode() {
    ...
    $friend_ids[] = $current_user_id; // Include the current user

    $leaderboard = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mk_players WHERE id IN (" . implode(',', $friend_ids) . ") ORDER BY score DESC");
...
function meiko_display_friend_requests_shortcode() {
    ...
    if (isset($_POST['accept_request'])) {
        ...
        $receiver_friends .= $receiver_friends ? ",$sender_id" : "$sender_id";
        ...
        $sender_friends .= $sender_friends ? ",$current_user_id" : "$current_user_id";
